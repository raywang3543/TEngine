...
│   ├── StacklandsStackRules.cs   # 允许堆叠的成对判定，以及配方/动作的需求与工人匹配（唯一实现）
...
│   ├── StacklandsBoardLayout.cs   # 牌面重叠统一解算器（顶开），Tick 末尾执行
│   └── ContentValidationReport.cs # 内容结构校验报告和缺失数据异常
└── View/
    ├── StacklandsBoardView.cs     # 2D 牌桌创建、输入、相机和快照渲染
    ├── CardView.cs                # 卡牌图形、文字、描边、进度、装备槽指示、层级和快照位置平滑跟随
...
`StacklandsBoardLayout` 是牌面重叠的统一解算器：以牌堆和卡包为实体按牌面 AABB（1.5×2.0）检测重叠，沿穿透较浅的轴把一方推出并留 0.1 间隙，由 `RunCtrl.Tick` 末尾统一执行。任何来源造成的重叠——拖放被拒、开卡包/月亮事件生成、读档残留——都走这一条路径，各业务路径不再分别判断。让路方向由 `LastActiveRevision` 决定：卡牌和卡包在创建或被拖动时记录当时的 `Run.Revision`，重叠对中较新（刚创建/刚移动）的一方让路，静止目标绝不被推；同堆成员不算重叠；敌我单位分属两堆时视为交战追击，豁免解算以免打断 `WorldCtrl` 的合堆触发。视图侧由 `CardView` 的快照位置平滑跟随把解算位移呈现为滑开动画。
...
- 协调工作、战斗、世界移动、Moon 和 HUD；Tick 末尾调用 `StacklandsBoardLayout.ResolveOverlaps` 统一顶开任何来源的牌面重叠。
- 执行 Profile/Run 保存以及存档错误流程。

### `StacklandsBoardCtrl`

- 处理单卡/整堆移动、合并、拆堆、兼容性判定和容量限制。
- 堆叠兼容性：拖到目标卡上时按 `StacklandsStackRules.CanStackOn` 判定，每张被拖卡都必须与命中的目标卡兼容；不兼容或超出 `MaxStackSize` 时提示并把被拖卡牌放到落点独立成新堆（不回弹原位），与目标残留的重叠由 `StacklandsBoardLayout` 在 Tick 末尾统一顶开，涉及它们的工作进度终止。
- 敌对单位（`UnitFaction.Hostile`/`Boss`）不可拖动：移动集合中含敌对单位时整体拒绝并提示"敌对单位无法拖动"，重新发布 Board 快照复位拖动表现；View 层按下敌对单位只发送 `SelectCard`，不进入拖动状态。
...
### `StacklandsStackRules`

- 允许堆叠的成对判定（对齐原作）：同一 CardId 同类归并；双方都是村民类（卡牌 `category = VILLAGER`，含职业、Dog、Baby）；双方都是装备类；装备与 `CanEquip` 单位的佩戴关系；敌我单位接触发起战斗；存在配方/动作交互关系（两卡可参与同一配方，或一方为动作来源、另一方匹配其原料或工人要求）。
- 整堆移动要求每张被拖卡都与落点命中的目标卡兼容；拖到空白处不做兼容性检查。
- `RequirementMatches`/`WorkerMatches` 是配方/动作需求匹配与工人匹配的唯一实现，`WorkCtrl` 委托调用；`GameModel.IsHostile` 同样委托此处的阵营判定。
...
- 需求匹配方式与 Luban `ECardMatcher` 对齐：`EXACT` 按卡牌 ID、`TAG` 按卡牌标签、`CATEGORY` 按卡牌分类；匹配实现委托 `StacklandsStackRules`。
...
- 月末按优先级喂食，食物不足时调用 `CombatCtrl.Kill`；被消耗的食物卡按单位分配进食配对，经 `CoreSystem.PublishFeeding` 通知表现层播放食物飞向单位的动画。
...
当前规则中，含敌对单位的移动整体被拒绝（复位回拖动前位置）；无归并或交互关系、或超出 `MaxStackSize` 的堆叠被拒绝，被拖卡牌放到落点独立成新堆，重叠由 `StacklandsBoardLayout` 统一顶开；合法堆叠只受容量限制，向工作中的牌堆堆叠会打断其进度。
...
MoonRemaining <= 0
  → WorldCtrl.EndMoon
  → 汇总单位与食物
  → 按 Baby、Dog、其他单位顺序决定喂食/挨饿名单
  → 消耗食物卡并按单位分配进食配对
  → PublishFeeding：View 播放食物飞向单位的动画（重合后食物卡销毁）
  → 食物不足的单位进入 Kill 流程
  → 检查卡牌上限
...
| `StacklandsFeeding` | `IReadOnlyList<FeedingSnapshot>` | 月末进食动画：被消耗食物卡 → 进食单位的配对 |
...
- 堆叠目标判定按被拖卡牌的碰撞体重叠（`Physics2D.OverlapBoxAll`，取重叠面积最大者），不按指针点命中；拖动途中实时以黄色边框高亮当前重叠命中的目标卡。
- 点击（位移不足 10px）可佩戴且已佩戴装备的单位时在单位下方展开装备卡堆；点击卡堆中的装备卡发送 `Unequip` 卸下对应槽位，再次点击该单位或点击其他位置则收起卡堆。
- 使用临时高 `SortingGroup` 层级显示正在拖动的单卡、整堆或卡包。
- 月末进食动画：`PlayFeeding` 把被消耗的食物卡视图移出快照管理，按进食配对顺序一张接一张飞向进食单位的实时位置（前一张消失后下一张才起飞），两卡重合后停留 0.35 秒再销毁食物卡视图；飞行期间关闭碰撞并置于最高层级。
- 只发送命令，不自行修改 Run 状态。
