...
- 堆叠兼容性：拖到目标卡上时按 `StacklandsStackRules.CanStackOn` 判定，每张被拖卡都必须与命中的目标卡兼容；不兼容则整体拒绝并提示"这些卡牌无法堆叠"，重新发布 Board 快照复位拖动表现。
...
当前规则中，含敌对单位的移动整体被拒绝；无归并或交互关系的卡牌不允许堆叠；其余移动只受 `MaxStackSize` 容量限制，向工作中的牌堆堆叠会打断其进度。拒绝时重新发布 Board 快照，使拖动表现回到原坐标。
