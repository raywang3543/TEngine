...
| 拖动卡牌 | 卡牌允许移动 | 卡牌跟随指针；原有合法工作通常被中断或重置 | 卡牌被移动或进入另一牌堆 | 释放位置非法时回到原位置附近 |
| 堆叠卡牌 | 拖动卡与目标卡发生有效重叠 | 合并为空间上的一个牌堆，并立即重新检查交互 | 合法组合开始计时；普通组合保持堆叠 | 组合不合法时不启动进度 |
...
- 无效组合可以保持空间堆叠，但不创建工作；若表现层不支持无效堆叠，则将被拖卡放回最近合法位置。
...
牌堆内部的精确命中区域、拖动中间卡时是否携带其上方子堆、无效释放的回弹动画属于**待实机验证**，不应固化为配方规则。
