...
> 整理日期：2026-08-03
> 2026-08-06 补充：按《Stacklands_Original版_装备与Mobs数值.md》补全装备与 Mobs 的攻速/命中/伤害/防御/特效数值（见 2.4、4.6、4.9）。
...
- 战斗数值按档位记录：命中 Small 60% / Normal 70% / High 80%；伤害 Weak 2 / Normal 3 / Very Strong 5；防御按格挡点数记（挡 0–2）；攻速 Fast 1s / Normal 1.5–2.3s / Slow 2.5s / Very Slow 3.5s。装备的 `+N` 表示在对应档位阶梯上上移 N 档（如 Damage 从 Weak(2) +1 → Normal(3)）。
...
`友好`表示不会主动触发敌对战斗，但仍有战斗属性。攻速/命中/伤害/防御/特效为 Wiki infobox 口径的档位数值，Combat Level（CL）不参与伤害计算。

| # | 卡牌 | 阵营 | CL / HP | 类型 | 攻速 | 命中 | 伤害 | 防御 | 特效 | 产出与逻辑 |
|---:|---|---|---|---|---|---|---|---|---|---|
| 1 | Bear（熊） | 敌对 | 22 / 25 | 近战 | Slow 2.5s | 60% | Normal 3 | 挡 2 | 10% 暴击 | 掉 Raw Meat（生肉）、Poop（粪便）、Bone（骨头），可能掉 Bear Claw（熊爪） |
| 2 | Chicken（鸡） | 友好 | 6 / 5 | 近战 | Normal 1.5s | 60% | Weak 2 | 挡 0 | 无 | 每 90s 产 Egg（鸡蛋）；Chicken（鸡） + Egg（鸡蛋）可孵出新 Chicken（鸡） |
| 3 | Cow（牛） | 友好 | 9 / 5 | 近战 | Normal 2.3s | 59% | Weak 2 | 挡 1 | 10% 眩晕目标 5s | 每 90s 产 Milk（牛奶） |
| 4 | Demon（恶魔） | Boss | 174 / 299 | 近战 | Fast 1s | 80% | Very Strong 5 | 挡 2 | 5% 眩晕全部敌方 5s | Temple（神庙） + Goblet 召唤 |
| 5 | Giant Rat（巨鼠） | 敌对 | 19 / 20 | 近战 | Normal 1.5s | 70% | Normal 3 | 挡 1 | 无 | 掉 Raw Meat（生肉）/Bone（骨头），可能掉 Rat Crown（鼠王冠） |
| 6 | Goblin（哥布林） | 敌对 | 8 / 10 | 近战 | Slow 2.5s | 70% | Weak 2 | 挡 0 | 无 | 可掉 Magic Dust（魔法粉尘）、Map（地图）或 Goblin Hat（哥布林帽） |
| 7 | Rabbit（兔） | 友好 | 8 / 5 | 近战 | Normal 1.5s | 60% | Weak 2 | 挡 0 | 10% 自身 Frenzy 10s | 每 90s 产 Poop（粪便）；可掉肉/胡萝卜等 |
| 8 | Rat（老鼠） | 敌对 | 6 / 5 | 近战 | Normal 1.5s | 60% | Weak 2 | 挡 0 | 无 | 主要掉 Raw Meat（生肉）或 Coin（金币） |
| 9 | Skeleton（骷髅） | 敌对 | 12 / 12 | 近战 | Slow 2.5s | 70% | Weak 2 | 挡 1 | 5% 暴击 | 可掉 Bone（骨头）、Magic Dust（魔法粉尘）或装备 |
| 10 | Slime（史莱姆） | 敌对 | 14 / 15 | 近战 | Normal 1.5s | 70% | Weak 2 | 挡 1 | 无 | 死亡后分裂为 3 个 Small Slime（小史莱姆） |
| 11 | Small Slime（小史莱姆） | 敌对 | 4 / 4 | 近战 | Very Slow 3.5s | 70% | Weak 2 | 挡 0 | 无 | 可掉 Coin（金币）、Egg（鸡蛋）、Magic Dust（魔法粉尘）、Key（钥匙） |
| 12 | Wicked Witch（邪恶女巫） | Boss/敌对 | 160 / 300 | 魔法 | Slow 2.5s | 70% | Normal 3 | 挡 2 | 无 | 来自 Dark Forest（黑暗森林）第 9 波且每局仅一次。虽列于 Original 页，实际逻辑跨到 Dark Forest（黑暗森林） |
| 13 | Wolf（狼） | 敌对 | 18 / 20 | 近战 | Normal 1.5s | 60% | Weak 2 | 挡 2 | 无 | Wolf（狼） + Bone（骨头） → Dog（狗）；可掉 Raw Meat（生肉）/Wolf Head（狼头） |

强度梯度：Friendly（棕卡）3 个全是低威胁经济动物；Hostile（红卡）从 Small Slime（CL 4）到 Demon（CL 174）。Demon（恶魔）与 Wicked Witch（邪恶女巫，CL 160）是两端 Boss。
...
三件装备均为 Hand 槽、近战类型，恰好对应 Original 仅有的三个村民职业。

| # | 卡牌 | 获得/配方 | 属性修正与转职 | 特效 | Item Level | 售价 |
|---:|---|---|---|---|---:|---:|
| 1 | Map（地图） | Goblin（哥布林）、Old Tome（古老典籍）点子全收集后的产物、商车、宝箱 | 伤害档位 -1，探索速度 ×2；Villager（村民） → Explorer（探险家） | 无 | 1 | 3 |
| 2 | Spear（长矛） | 1 Wood（木材） + 2 Stick（木棍），20s；商车/宝箱 | 伤害档位 +2；Villager（村民） → Militia（民兵） | 5% 暴击 | 3 | 3 |
| 3 | Sword（剑） | 1 Iron Bar（铁锭） + 2 Stick（木棍），15s；宝箱等 | 攻速、命中、伤害各 +1 档；Villager（村民） → Swordsman（剑士） | 10% 使目标流血 | 5 | 3 |

强度对照：Sword（剑）（Item Level 5）武装的 Swordsman（剑士）面板约为 Damage Normal(3) / 2.3s / 77% 命中，刚好覆盖 Giant Rat（巨鼠）、Wolf（狼）、Bear（熊）这一档（CL 18–22）的中期压力；Demon（恶魔）则靠人海与 Stun 类装备堆叠应对。
...
- 《Stacklands_Original版_装备与Mobs数值.md》：装备与 Mobs 的攻速/命中/伤害/防御/特效档位数值，2026-08-06 并入 2.4、4.6、4.9；其本身筛选自《Stacklands全Mobs战斗数值》《Stacklands全装备数值》两份全量表。
