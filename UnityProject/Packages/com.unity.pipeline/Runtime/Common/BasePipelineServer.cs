...
namespace Unity.Pipeline
{
    ...
    public abstract class BasePipelineServer
    {
        ...
        /// <summary>
        /// Bind explicit loopback prefixes.
        ///
        /// Mono's HttpListener on macOS can reject the top-level wildcard prefix
        /// "http://+:{port}/". When that happens every availability probe fails and the caller
        /// misleadingly reports that the entire port range is occupied. Explicit loopback
        /// prefixes avoid the wildcard binding and keep the server local-only.
        /// </summary>
